...
def textract_chunking(content: list[dict[str, Any]], chunk_size: int = 1000) -> list[dict[str, Any]]:
    ...
    Returns list of chunks with 'text' and 'pages' keys.
    """
    chunks = []
    current_chunk = []
    current_length = 0
        ...
        if args.output_path:
            output_data = {
                "chunks": [
                    {
                        "text": entry.string,
                        "pages": [idx.dict() for idx in entry.index_numbers] if entry.index_numbers else None,
                        "metadata": entry.ingestion.dict() if entry.ingestion else None
                    }
            ...
            print(f"Saved chunks to {args.output_path}")
