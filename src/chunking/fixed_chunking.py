...
@FunctionRegistry.register("chunk", "fixed_length_chunking")
async def fixed_chunks(document: list[Document], **kwargs) -> list[Entry]:
    ...
    for doc in document:
        ...
        chunks = fixed_length_chunking(content, chunk_size=chunk_size, provider=provider, model=model)
        formatted_entries = chunks_to_entries(doc, chunks, "embedding", chunking_metadata)
        doc.entries = formatted_entries
        new_docs.append(doc)
    return new_docs

def fixed_length_chunking(content: list[dict[str, Any]], chunk_size=500, provider=None, model=None) -> list[dict[str, Any]]:
    """
    ...
    if provider:
        ...
        for i in range(0, len(word_positions), chunk_size):
