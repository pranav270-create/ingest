...
from src.sql_db.etl_model import Ingest, ProcessingStep, Entry, ProcessingPipeline, ingest_pipeline
import hashlib
    ...
    try:
        ...
        if pipeline:
            ...
            for ingest in ingests.values():
                rel_stmt = mysql_insert(ingest_pipeline).values({
                    "ingest_id": ingest.id,
                    "pipeline_id": pipeline.id
                })
                rel_stmt = rel_stmt.on_duplicate_key_update(
                    ingest_id=rel_stmt.inserted.ingest_id,
                    pipeline_id=rel_stmt.inserted.pipeline_id
                )
...
async def clone_pipeline(session: AsyncSession,
    ...
    for ingest in ingests:
        ...
        stmt = mysql_insert(ingest_pipeline).values({
            "ingest_id": ingest.id,
            "pipeline_id": new_pipeline.id
        })
