...
import regex
from marker.schema.merged import FullyMergedBlock, MergedBlock, MergedLine
    ...
    for page in pages:
        ...
        for blocknum, block in enumerate(page.blocks):
            ...
            if len(block_lines) > 0:
                page_blocks.append(MergedBlock(lines=block_lines, pnum=block.pnum, bbox=block.bbox, block_type=block.block_type))
...
def merge_lines(blocks: list[list[MergedBlock]]) -> list[FullyMergedBlock]:
    ...
    for idx, page in enumerate(blocks):
        for block in page:
            ...
            if block_type != prev_type and prev_type:
                text_blocks.append(
                    FullyMergedBlock(text=block_surround(block_text, prev_type), block_type=prev_type, pnum=current_pnums)
                )
                block_text = ""
    ...
    text_blocks.append(FullyMergedBlock(text=block_surround(block_text, prev_type), block_type=block_type, pnum=current_pnums))
    return text_blocks
