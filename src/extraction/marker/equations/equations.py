...
from marker.schema.block import Block, Line, Span, find_insert_block, split_block_lines
...
def increment_insert_points(page_equation_blocks, insert_block_idx, insert_count):
    for idx, (block_idx, _, _, _, _) in enumerate(page_equation_blocks):
...
def insert_latex_block(page_blocks: Page, page_equation_blocks, predictions, pnum, processor):
    ...
    success_count = 0
    fail_count = 0
    for block_number, (insert_block_idx, insert_line_idx, token_count, block_text, equation_bbox) in enumerate(
...
def replace_equations(doc, pages: list[Page], texify_model, batch_multiplier=1):
    ...
    for page_idx, page_equation_blocks in enumerate(equation_blocks):
        ...
        for equation_idx, (insert_block_idx, insert_line_idx, token_count, block_text, equation_bbox) in enumerate(
            page_equation_blocks
        ):
    ...
    for page_idx, page_equation_blocks in enumerate(equation_blocks):
        ...
        success_count, fail_count, converted_span = insert_latex_block(
            pages[page_idx], page_equation_blocks, page_predictions, page_idx, texify_model.processor
        )
